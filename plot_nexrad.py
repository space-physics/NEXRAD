...
# %% Process / Plot
    if isinstance(P.keo, list) and len(P.keo) == 2:
        ofn = nexrad_keogram(flist, P.keo, P.wld, odir, scalefn=scalefn)
        print('keogram created at', ofn)
    else:
        nexrad_loop(flist, P.wld, odir, P.lattick, scalefn=scalefn)
        if odir:
            print('\nImageMagick can convert the PNGs to animated GIF by a command like:')
            print('\nconvert map2018-0101T09*.png out.gif')


def nexrad_keogram(flist: List[Path],
                   keo: List[str],
                   wld: Path,
                   odir: Path=None,
                   scalefn: Path=None) -> Optional[Path]:
...
def nexrad_loop(flist: List[Path],
                ...
                lattick: float=None, scalefn: Path=None):
