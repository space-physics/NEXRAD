...
def nexrad_panel(
    flist: List[Path], wld: Path, ofn: Optional[Path], lattick: float = None, scalefn: Path = None
):
    if figure is None:
...
def overlay2d(
    ...
    if (
        "fg" in mlp
        and fignum_exists(mlp["fg"].number)
        and "himg" in mlp
        and mlp["himg"] is not None
    ):
    ...
    gl = ax.gridlines(
        crs=GREF, draw_labels=True, linewidth=1, color="gray", alpha=0.5, linestyle="--"
    )
...
def keogram(keo: xarray.DataArray, ofn: Path = None, scalefn: Path = None):
